...
my.other.genes <- c("FKBP5");
for (gene in c(other.genes, yet.other.genes)) {
	print(gene);
	try({
...
pik <- intersect(
	grep("PIK", rownames(expr), value=TRUE),
	grep("PIK", rownames(cn), value=TRUE)
);

g <- ggplot(make_genes_expr_cn(c(pik, "PTEN")), aes(x=log2(cn), y=expr, colour=met_type)) + 
