...
out.fname <- filename("pkb-tcga-luad", tag=c("stage3", "gpldiff"));
...
idx <- which(results$ldiff > 0.5 & results$fdr < 0.025 & results$gistic_q < 0.025);
print(results[idx, ])

# FIXME fragile!
# label significant results
results$cytoband <- NA;
results$cytoband[idx] <- c("11q22.2", "8q24.21", NA,"9p21.3");
