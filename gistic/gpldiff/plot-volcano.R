...
idx <- which(results$ldiff > 0.5 & results$fdr < 0.01);
print(results[idx, ])

# FIXME fragile!
# label significant results
results$cytoband <- NA;
results$cytoband[idx] <- c("4q31.23", "8q24.21", "11q22.2", "9p21.3");
...
qdraw(
	{
		ggplot(results, aes(x=diff, y=fdr, colour=type, label=cytoband)) +
			...
			geom_hline(yintercept=0.01, colour="grey40", linetype="dashed") +
			geom_point() + geom_label_repel() + theme_bw() +
			theme(panel.grid=element_blank()) + guides(label=FALSE) +
