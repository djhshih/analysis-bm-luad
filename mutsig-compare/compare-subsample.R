...
rank.le <- ranks <= case$rank;
rank.le.p <- rowMeans(rank.le, na.rm=TRUE);
sort(pmax(rank.le.p[sig.idx], 1/B))
rank.le.sel.bh <- sort(p.adjust(pmax(rank.le.p[sig.idx], 1/B), "BH"))

qdraw(
	qplot(rank.le.p, geom="histogram", bins=100) + theme_bw()
	,
	file = insert(out.fname, c("mutsigcv2-p", boot.type, "rank-le", "hist"), ext="pdf")
);

qdraw(
	p_qqplot(rank.le.p) + theme_bw()
	,
	file = insert(out.fname, c("mutsigcv2-p", boot.type, "rank-le", "qq"), ext="pdf")
);

rank.le.p.lambda <- inflation_factor(rank.le.p[rank.le.p < 1]);
rank.le.p.lambdas <- inflation_factor(rank.le.p[rank.le.p < 1], seq(0, 1, 0.1));

rank.le.p.deflated <- pmin(rank.le.p * rank.le.p.lambda, 1);

qplot(rank.le.p.deflated[rank.le.p.deflated < 1], geom="histogram", bins=11) + theme_bw()

p_qqplot(rank.le.p.deflated[rank.le.p.deflated < 1]) + theme_bw()
...
qdraw(
	...
	file = insert(out.fname, c("mutsigcv2-p", boot.type, "p-ge", "qq"), ext="pdf")
);
...
compare_plot <- function(case, control, genes) {
...
qdraw(
	qdraw(compare_plot(make_mutsig2cv(boot$p[,1]), control, genes), height=7, width=7.5)
	,
	file = insert(out.fname, c("compare-boot", boot.type, "b1", "mutsig2cv-rank"), ext="pdf")
);

qdraw(
	qdraw(compare_plot(make_mutsig2cv(boot$p[,2]), control, genes), height=7, width=7.5)
	,	
	file = insert(out.fname, c("compare-boot", boot.type, "b2", "mutsig2cv-rank"), ext="pdf")
);

qdraw(
	qdraw(compare_plot(make_mutsig2cv(boot$p[,3]), control, genes), height=7, width=7.5)
	,
