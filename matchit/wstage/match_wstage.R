...
pkb.clin.sel <- select(pkb.clin, clinical_id, sample_id, age_at_primary_dx, sex, smoking_signature_high, exac_pop, purity, stage_clean) %>% mutate(cohort="BM-LUAD");
tcga.clin.sel <- select(tcga.clin, clinical_id, sample_id, age_at_primary_dx, sex, smoking_signature_high, exac_pop, purity, stage_clean) %>% mutate(cohort="TCGA-LUAD");
...
qdraw(
	ggplot(clin.sel, aes(x=exac_pop, y=stage_clean, shape=sex, colour=cohort)) +
		geom_jitter(alpha=0.6) + theme_bw() +
		facet_grid(smoking_signature_high ~ .) +
		geom_vline(xintercept=seq(1.5, 6, 1), colour="grey60") +
		geom_hline(yintercept=seq(1.5, 5, 1), colour="grey60") +
		theme(panel.grid.major = element_blank()) +
		scale_colour_manual(values=cols)
	,
	width = 8, height = 5,
	file = insert(pdf.fname, c("exac-pop", "stage", "smoking"))
);

nona.idx <- complete.cases(select(clin.sel, sex, smoking_signature_high, exac_pop, cohorti, purity, stage_clean));
clin.sel.nona <- clin.sel[nona.idx, ];

mi <- matchit(cohorti ~ sex + exac_pop + smoking_signature_high + purity + stage_clean,
	data = select(clin.sel.nona, sex, smoking_signature_high, exac_pop, cohorti, purity, stage_clean),
	method="cem");
print(mi)
# only 15 cases (and 51 controls) are matched when matching on:
# sex, exac_pop, smoking_signature_high, purity, and stage_clean

groups <- c("Unmatched", "Matched");
nn <- melt(mi$nn, varnames=c("group", "cohort"));
qdraw(
	ggplot(filter(nn, group %in% groups) %>% mutate(group = factor(group, groups)), aes(x=cohort, y=value, fill=group)) +
		geom_bar(stat="identity") + theme_bw()
	,
	width = 3,
	file = insert(pdf.fname, c("purity", "stage", "matched", "sample-size"))
);

# therefore, do only stage matching

mi <- matchit(cohorti ~ sex + exac_pop + smoking_signature_high + stage_clean,
	data = select(clin.sel.nona, sex, smoking_signature_high, exac_pop, cohorti, stage_clean),
	method="cem");
print(mi)
...
qdraw(
	...
	,
	width = 3,
	file = insert(pdf.fname, c("stage", "matched", "sample-size"))
);
...
qdraw(
	ggplot(clin.sel.m, aes(x=exac_pop, y=stage_clean, colour=cohort, alpha=log(weight+1))) +
	...
	,
	width = 8, height = 5,
	file = insert(pdf.fname, c("exac-pop", "stage", "sex", "smoking", "cem", "alpha"))
);

qdraw(
	ggplot(clin.sel.m, aes(x=exac_pop, y=stage_clean, colour=cohort, size=weight)) +
		geom_jitter(alpha=0.6) + theme_bw() +
		facet_grid(sex ~ smoking_signature_high) +
		geom_vline(xintercept=seq(1.5, 6, 1), colour="grey60") +
		geom_hline(yintercept=seq(1.5, 5, 1), colour="grey60") +
	...
	,
	width = 8, height = 5,
	file = insert(pdf.fname, c("exac-pop", "stage", "sex", "smoking", "cem", "size"))
);

qdraw(
	ggplot(clin.sel.m, aes(x=exac_pop, y=stage_clean, colour=cohort, size=weight, alpha=log(weight+1))) +
		geom_jitter() + theme_bw() +
		facet_grid(sex ~ smoking_signature_high) +
		geom_vline(xintercept=seq(1.5, 6, 1), colour="grey60") +
		geom_hline(yintercept=seq(1.5, 5, 1), colour="grey60") +
	...
	,
	width = 8, height = 5,
	file = insert(pdf.fname, c("exac-pop", "stage", "sex", "smoking", "cem", "size", "alpha"))
);
...
for (matching in names(clin.sel.matching)) {
	...
	clin.sel <- mutate(clin.sel.matching[[matching]], stage = stage_clean);
	...
	qdraw(
		ggplot(clin.sel, aes(x=cohort, fill=stage_clean)) +
			geom_bar() + theme_bw() +
			xlab("") + ylab("Count")
		,
		width = 3,
