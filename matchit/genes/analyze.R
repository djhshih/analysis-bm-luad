...
mut.genes <- c("KRAS", "TP53", "STK11", "KEAP1", "EGFR");
...
qdraw(
	ggplot(mut.genes.coefs, aes(x=cohort, y=mean, ymin=lower, ymax=upper, fill=cohort)) + 
		...
		facet_grid(. ~ factor(gene, levels=mut.genes)) +
...
qdraw(
	ggplot(mut.genes.coefs, aes(x=cohort, y=mean, ymin=lower, ymax=upper, colour=cohort)) + 
		geom_point() +
	...
	,
	width = 4, height = 3,
	file = insert(pdf.fname, c("snv-freq", "bar", "vdot"))
);
...
qdraw(
	ggplot(mut.genes.coefs, aes(x=cohort, y=mean, ymin=lower, ymax=upper, colour=cohort)) + 
		...
		facet_wrap(~ factor(gene, levels=mut.genes), ncol=1, strip.position="top") +
		scale_colour_manual(values=cols.ch) + 
		ylab("Somatic SNV frequency") + xlab("") +
		ylim(0, 1)
	,
	width = 4, height = 4,
	file = insert(pdf.fname, c("snv-freq", "dot"))
);
...
qwrite(amp.genes.coefs, insert(tsv.fname, "amp-freq"));
...
qdraw(
	ggplot(amp.genes.coefs, aes(x=cohort, y=mean, ymin=lower, ymax=upper, colour=cohort)) + 
		geom_point() +
		geom_errorbar(width=0.1) + theme_bw() + coord_flip() +
	...
	,
	width = 5, height = 5,
	file = insert(pdf.fname, c("amp-freq", "dot"))
);
...
qwrite(del.genes.coefs, insert(tsv.fname, "del-freq"));
...
qdraw(
	ggplot(del.genes.coefs, aes(x=cohort, y=mean, ymin=lower, ymax=upper, colour=cohort)) + 
		geom_point() +
		geom_errorbar(width=0.1) + theme_dot() + coord_flip() +
		#facet_wrap(~ gene, scales="free_y") +
		facet_wrap(~ gene, ncol=1) + ylim(0, 0.55) +
		scale_colour_manual(values=cols.ch) + 
		ylab("Somatic hom. del. frequency") + xlab("")
	,
	width = 4, height = 2,
	file = insert(pdf.fname, c("del-freq", "dot"))
);
...
qwrite(cna.genes.coefs, insert(tsv.fname, "cna-freq"));
...
qdraw(
	ggplot(cna.genes.coefs, aes(x=cohort, y=mean, ymin=lower, ymax=upper, fill=cohort)) + 
		...
		facet_grid(. ~ gene) + 
		scale_fill_manual(values=cols.ch) + 
		ylab("Somatic CNA frequency") + xlab("") + ylim(0, 0.35)
	,
	width = 5, height = 3,
...
qdraw(
	ggplot(cna.genes.coefs, aes(x=cohort, y=mean, ymin=lower, ymax=upper, colour=cohort)) + 
		geom_point() +
		geom_errorbar(width=0.2) + theme_vdot() + 
		facet_grid(. ~ gene) + 
		scale_colour_manual(values=cols.ch) + 
		ylab("Somatic CNA frequency") + xlab("") + ylim(0, 0.35)
	,
	width = 4, height = 3,
	file = insert(pdf.fname, c("cna-freq", "vdot"))
);
...
qdraw(
	...
	,
	width = 4, height = 4,
	file = insert(pdf.fname, c("cna-freq", "dot"))
);
...
names(tma.amp.genes) <- tma.amp.genes;
...
qwrite(cna.genes.coefs, insert(tsv.fname, c("tma", "amp-freq")));
...
qdraw(
	ggplot(tma.amp.genes.coefs, aes(x=cohort, y=mean, ymin=lower, ymax=upper, fill=cohort)) + 
		...
		ylab("Somatic amplification frequency") + xlab("") + ylim(0, 0.35)
	,
	width = 3, height = 3,
...
qdraw(
	ggplot(tma.amp.genes.coefs, aes(x=cohort, y=mean, ymin=lower, ymax=upper, colour=cohort)) + 
		geom_point() +
		geom_errorbar(width=0.1) + theme_vdot() + 
		facet_wrap(~ gene, scales="free_y") +
		scale_colour_manual(values=unname(cols.ch)) + 
		ylab("Somatic amplification frequency") + xlab("") + ylim(0, 0.35)
	,
	width = 2, height = 3,
	file = insert(pdf.fname, c("tma", "amp-freq", "vdot"))
);
...
qdraw(
	...
	,
	width = 4, height = 2,
	file = insert(pdf.fname, c("tma", "amp-freq", "dot"))
);
...
qwrite(cna.genes.coefs, insert(tsv.fname, c("tma2", "amp-freq")));
...
qdraw(
	ggplot(tma2.amp.genes.coefs, aes(x=cohort, y=mean, ymin=lower, ymax=upper, colour=cohort)) + 
		geom_point() +
		geom_errorbar(width=0.2) + theme_vdot() + 
		facet_wrap(~ gene, scales="free_y") +
		scale_colour_manual(values=unname(cols.ch)) + 
	...
	,
	width = 2, height = 3,
	file = insert(pdf.fname, c("tma2", "amp-freq", "vdot"))
);
...
qdraw(
	...
	,
	width = 4, height = 2,
	file = insert(pdf.fname, c("tma2", "amp-freq", "dot"))
);
...
qdraw(
	ggplot(combined.amp.genes.coefs, aes(x=cohort, y=mean, ymin=lower, ymax=upper, colour=cohort)) + 
		geom_point() +
		geom_errorbar(width=0.1) + theme_dot() + coord_flip() +
		facet_wrap(~ gene, ncol=1) +
		#scale_fill_manual(values=unname(cols.ch)) + 
	...
	,
	width = 4, height = 4,
	file = insert(pdf.fname, c("tma-combined", "amp-freq", "dot"))
);
